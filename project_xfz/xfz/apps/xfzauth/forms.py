...
class RegisterForm(forms.Form,FormMixin):
    ...
    def clean(self):
        ...
        telephone = cleaned_data.get('telephone')            #这个是自己生成的
        sms_captcha = cleaned_data.get('sms_captcha')        #这个是用户提交的
        cached_sms_captcha = cache.get(telephone)
        # if not cached_sms_captcha or cached_sms_captcha.lower() != sms_captcha.lower():
        #     raise forms.ValidationError("短信验证码错误")

        exists = User.object.filter(telephone=telephone).exists()
        if exists:
            raise forms.ValidationError('该手机号码已经被注册了!')
    ...
    def get(self,request):
        ...
        if start or end:
            if start:
                ...
                start_date = datetime(year=2019,month=3,day=6)
...
def delete_news_category(request):
    ...
    # print(pk)
    try:
        NewsCategory.objects.filter(pk=pk[0]).delete()
        return restful.ok()
    except:
        return restful.unauth_errors(message="该分类不存在！")
    # if NewsCategory.objects.filter(pk=pk).delete():
    #     return restful.ok()
    # else:
...
@staff_member_required(login_url='index')
def qntoken(request):
    access_key = "Vmhomy9CMEcrjVQmou0CydtWax1_M1xqUTDdw6zy"
    secret_key = "JrInPczmw3Ss74tUu_Ud6Xf7Hymx777N3d25ywNL"

    bucket = "hyvideo"
    q = qiniu.AccountClient(access_key,secret_key)
